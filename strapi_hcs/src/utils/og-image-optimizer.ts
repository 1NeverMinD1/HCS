...
async function cropToRatio(inputBuffer: Buffer, ratio: number): Promise<Buffer> {
...
export async function generateSeoImageCrops(
  ...
  try {
    ...
    if (!source && primaryField) source = await getFileEntity(strapi, result[primaryField]);
    if (!source && fallbackField) source = await getFileEntity(strapi, result[fallbackField]);
    if (!source) return;
    ...
    for (const [field, ratio] of Object.entries(SEO_CROPS)) {
      ...
      const tmpPath = path.join(os.tmpdir(), `${field}_${documentId}_${crypto.randomUUID()}.jpg`);
      ...
      const uploaded = await uploadService.upload({
        ...
        files: {
          path: tmpPath,
          name: `${field}_${marker}${documentId}.jpg`,
          type: "image/jpeg",
    ...
    if (Object.keys(cropIds).length === 0) return;
    ...
    strapi.log.error(`[seo-image-crops] ${uid} #${documentId}: ошибка — ${err.message}`);
  } finally {
